...
в таблицу `app_state`. Это сохраняет совместимость с текущей бизнес-логикой и снижает риск регрессий. Нормализация в отдельные таблицы остается следующим этапом после проверки production runtime на Railway.
...
Задать `DATABASE_URL` в Railway Variables, сделать redeploy и проверить, что:

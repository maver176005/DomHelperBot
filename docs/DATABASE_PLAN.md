...
в таблицу `app_state`. Это сохраняет совместимость с текущей бизнес-логикой и снижает риск регрессий. `DATABASE_URL` включен в Railway Variables, бот запущен и работает. Railway `Serverless` выключен у bot service и Postgres, чтобы polling-бот и БД не уходили в sleep. Нормализация в отдельные таблицы остается следующим этапом после проверки сохранения данных через redeploy.
...
- `rating_client_user_id text references users(id)`
...
4. Выключить `Serverless` у bot service и Postgres.
5. Оставить `Wait for CI` включенным для deploy после зеленого GitHub Actions.
6. Redeploy.
7. Проверить logs:
   - миграции прошли;
...
8. Проверить в Telegram:
   ...
   - создание заказа;
   - создание listing;
   - оценка исполнителя после подтверждения заказа;
